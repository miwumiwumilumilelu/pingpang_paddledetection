...
def predict_image(detector, image_list, result_path, threshold):
    ...
    for index in range(len(image_list)):
        ...
        if im_bboxes_num > 0:
            # 提取所有有效框的索引（score >= threshold）
            valid_indices = np.where(det_results['boxes'][:, 1] >= threshold)[0]

            if len(valid_indices) == 0:
                continue  # 无满足阈值的框，跳过

            # 按 score 降序排序，取第一个（最高分）
            sorted_indices = valid_indices[np.argsort(-det_results['boxes'][valid_indices, 1])]
            best_idx = sorted_indices[0]

            # 获取最高分框的信息
            bbox = det_results['boxes'][best_idx, 2:]  # x1, y1, x2, y2
            score = det_results['boxes'][best_idx, 1]
            class_id = int(det_results['boxes'][best_idx, 0])
            ...
            c_results["result"].append({
                "image_id": image_id,
                "type": detector.pred_config.labels[class_id],
                "x": x_center,
                "y": y_center,
                "width": width,
                "height": height,
